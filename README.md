...
### 7.1 代码生成 — 数据库 `(/ops/codegen/database)`

#### `GET /ops/codegen/database/list`
...
#### `GET /ops/codegen/database/tableInfoList`
...
#### `GET /ops/codegen/database/downloadCode`
...
### 7.2 代码生成 — JSON/XML `(/ops/codegen/jsonxml)`

#### `POST /ops/codegen/jsonxml/genCode`
...
#### `POST /ops/codegen/jsonxml/downloadCode`
...
### 7.3 代码生成 — Swagger/OpenAPI `(/ops/codegen/swagger)`

#### `GET /ops/codegen/swagger/downloadCodeForVue3`
...
#### `GET /ops/codegen/swagger/downloadCodeForJmeter`
...
A：数据库连接通过 `uw-dao` 框架的连接池管理配置，连接池名称（`connName`）在 Nacos 配置中心的 `uw-code-center` 配置文件中定义，无需修改代码。添加新连接后，`/ops/codegen/database/list` 接口会自动包含新的连接名称。
